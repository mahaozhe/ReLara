...
from RLEnvs.MyMujoco.utils import *
...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        self.random_tgt = random_tgt
        self.reward_type = reward_type
        self.goal_dist_th = goal_dist_th
        self.tgt_speed = np.random.uniform(0.5, 5) if random_tgt else tgt_speed
        ...
        if exclude_current_positions_from_observation:
            observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(17,), dtype=np.float64)
        else:
            observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(18,), dtype=np.float64)

        MujocoEnv.__init__(self, "walker2d.xml", 4, observation_space=observation_space, **kwargs)
        obs = self._get_obs()
        self.observation_space = spaces.Dict(
            dict(
                desired_goal=spaces.Box(-np.inf, np.inf, shape=obs["achieved_goal"].shape, dtype="float64"),
    ...
    def _get_obs(self):
        ...
        if self.task == "height":
            achieved_goal = np.array([observation[0]])  # z position
            desired_goal = np.array([self.tgt_height])
        elif self.task == "speed":
            achieved_goal = np.array([observation[5]])  # x velocity
            desired_goal = np.array([self.tgt_speed])
        return {
    ...
    def reset_model(self):
        ...
        if self.task == "speed":
            self.tgt_speed = gen_rand_num_within(0.5, 5) if self.random_tgt else self.tgt_speed

...
class HumanoidStandupEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, reward_type="sparse", height_th=0.6, **kwargs):
        self._reward_type = reward_type
        self._height_th = height_th

        observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(376,), dtype=np.float64)
        MujocoEnv.__init__(self, "humanoidstandup.xml", 5, observation_space=observation_space, **kwargs)
